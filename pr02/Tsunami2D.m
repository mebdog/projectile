    ...
    for j = 1:n
        for i = 1:n
            if i == 1
                ...
                uFut(i+1,j) = leftBoundary(t, deltaT);
            % Corner Cases
            elseif i == n && j == 1
                uFut(i,j) = constantTop(i,j)*(2*uCur(i,j)-uCur(i-1,j)) + constantBottom(i,j)*uCur(i-1,j) + ...
                            constantLeft(i,j)*(2*uCur(i,j)-uCur(i,j+1)) + constantRight(i,j)*uCur(i,j+1) + ...
                            constantCurrent(i,j)*uCur(i,j) - uPast(i,j);
            elseif i == n && j == n
                uFut(i,j) = constantTop(i,j)*(2*uCur(i,j)-uCur(i-1,j)) + constantBottom(i,j)*uCur(i-1,j) + ...
                            constantLeft(i,j)*uCur(i,j-1) + constantRight(i,j)*(2*uCur(i,j)-uCur(i,j-1)) + ...
                            constantCurrent(i,j)*uCur(i,j) - uPast(i,j);
            % Edge Cases
            elseif i == n
                uFut(i,j) = constantTop(i,j)*(2*uCur(i,j)-uCur(i-1,j)) + constantBottom(i,j)*uCur(i-1,j) + ...
